...
type GoodsOptionsCommonAddUpdate struct {
	...
	Name    string `json:"name"             description:"商品规格名称" v:"required#名称必填"`
	Price   uint   `json:"price"            description:"价格 单位分"  v:"required#价格必填"`
	Stock   int    `json:"stock"            description:"库存"` //不为uint，是因为可能存在超卖的情况
}
...
type GoodsOptionsGetListCommonReq struct {
	g.Meta `path:"/goods/options/list" method:"get" tags:"商品规格" summary:"查询商品规格列表"`
	CommonPaginationReq
}
