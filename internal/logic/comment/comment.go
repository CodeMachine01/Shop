...
// 兼容处理：优先根据评论id删除，评论id为0 再根据对象id和type删除
...
func (*sComment) GetList(ctx context.Context, in model.CommentListInput) (out *model.CommentListOutput, err error) {
	...
	out = &model.CommentListOutput{
		Page: in.Page,
		Size: in.Size,
		List: []model.CommentListOutputItem{}, //提前实例化避免查不到数据返回为null而不是空数组
	}
	// 翻页查询
...
// 抽取方法 判断当前用户是否评论 for 商品详情&文章详情
func CheckIsComment(ctx context.Context, in model.CheckIsCommentInput) (bool, error) {
	condition := g.Map{
		dao.CommentInfo.Columns().UserId:   ctx.Value(consts.CtxUserId),
