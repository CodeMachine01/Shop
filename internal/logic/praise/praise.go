...
func (*sPraise) DeletePraise(ctx context.Context, in model.DeletePraiseInput) (res *model.DeletePraiseOutput, err error) {
	...
	if in.Id != 0 {
		...
		in.UserId = gconv.Uint(ctx.Value(consts.CtxUserId))
		id, err := dao.PraiseInfo.Ctx(ctx).OmitEmpty().
			Where(in).Delete()
		if err != nil {
...
func (*sPraise) GetList(ctx context.Context, in model.PraiseListInput) (out *model.PraiseListOutput, err error) {
	...
	out = &model.PraiseListOutput{
		Page: in.Page,
		Size: in.Size,
		List: []model.PraiseListOutputItem{}, //提前实例化避免查不到数据返回为null而不是空数组
	}
	// 翻页查询
...
func CheckIsPraise(ctx context.Context, in model.CheckIsPraiseInput) (bool, error) {
	condition := g.Map{
